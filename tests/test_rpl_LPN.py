...
def test_parent_selection(sim_engine):
    sim_engine = sim_engine(
        diff_config = {
            ...
            "rpl_of_weights" :  [0.3,0.7],
            'exec_numSlotframesPerRun': 1500,
            #'sf_class'       : 'MSF', #SFNone #MSF
...
# =========================== Network Joining =========================================
    ...
    # Re-enable the link between mote 1 and mote 4
    connectivity_matrix.set_pdr_both_directions(
        mote_1.id, mote_4.id, channel, 1.0
    )
    ...
    # Modify the link between mote 4 and DAGROOT
    connectivity_matrix.set_pdr_both_directions(
        mote_5.id, mote_4.id, channel, 1.0
    )
    ...
    # degrade link PDRs to 0.5*ACCEPTABLE_LOWEST_PDR
    # - between mote 0 and mote 4
    # - between mote 4 and mote 5
    connectivity_matrix.set_pdr_both_directions(
        mote_0.id,
    ...
    #assert mote_5.rpl.of.preferred_parent['mote_id'] == mote_3.id
    #assert mote_4.rpl.of.preferred_parent['mote_id'] == mote_1.id
    ...
    assert mote_3.rpl.get_rank() == 1024
    assert mote_4.rpl.get_rank() == 768
    assert mote_5.rpl.get_rank() == 1024 
...
# =========================== UL/DL Traffic Mote 5 =========================================
    ...
    for _ in range(10):
            ...
            assert datamote1.rpl.of.preferred_parent['mote_id'] == mote_4.id
            assert datamote2.rpl.of.preferred_parent['mote_id'] == mote_1.id

            assert connectivity_matrix.get_pdr(datamote1.id,mote_4.id,channel) == 1.0
            assert connectivity_matrix.get_pdr(datamote2.id,mote_1.id,channel) == 1.0
            assert connectivity_matrix.get_rssi(datamote1.id,mote_4.id,channel) == -79
            assert connectivity_matrix.get_rssi(datamote2.id,mote_1.id,channel) == -79
